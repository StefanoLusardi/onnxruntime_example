...
int main(int argc, char** argv)
{
    ...
    std::vector<float> outputTensorValues_f(outputTensorSize);
    std::vector<uint8_t> outputTensorValues_u8(outputTensorSize);
    auto get_output_tensor_values = [&]() -> std::tuple<void*, uint>
    {

        switch(outputType)
        {
            case ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT:
                return { static_cast<void*>(outputTensorValues_f.data()), sizeof(float) * outputTensorSize };

            case ONNX_TENSOR_ELEMENT_DATA_TYPE_UINT8:
                return { static_cast<void*>(outputTensorValues_u8.data()), sizeof(uint8_t) * outputTensorSize};
    ...
    auto [outputTensorValues, outputTensorSizeInBytes] = get_output_tensor_values();


    std::vector<Ort::Value> outputTensors;
    outputTensors.push_back(Ort::Value::CreateTensor(memoryInfo, outputTensorValues, outputTensorSizeInBytes, outputDims.data(), outputDims.size(), outputType));
    ...
    // auto out_begin = reinterpret_cast<float>(outputTensorValues);
    const float* buf = (float*)outputTensorValues;
    std::vector<float> out = {buf, buf + outputTensorSize};

    auto best_prediction = std::max_element(out.begin(), out.end());
    std::cout << "\n\nprediction_score: " << *best_prediction << std::endl;

    auto prediction_index = std::distance(out.begin(), best_prediction);
